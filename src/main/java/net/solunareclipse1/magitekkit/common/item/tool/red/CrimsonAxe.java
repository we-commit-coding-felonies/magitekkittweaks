...
public class CrimsonAxe extends AxeItem implements ICapabilityItem, IModeChanger, IEmpowerItem, IExtraFunction, IStaticSpeedBreaker {
	public CrimsonAxe(Tier tier, int damage, float speed, Properties props) {
	...
	public void appendHoverText(ItemStack stack, Level level, List<Component> tips, TooltipFlag flags) {
		...
		tips.add(new TranslatableComponent("tip.mgtk.crimson.axe").withStyle(ChatFormatting.UNDERLINE)); // Flavor
		tips.add(new TranslatableComponent("tip.mgtk.crimson.axe.1", funcKeyText)); // Keys
